...
2. **Run phase** (`action/agents/<name>/run.sh`):
   ...
   - **Expected behavior:**
     ...
     - **Agent output must include code changes in markdown blocks**
   ...
   - **Agent output format (IMPORTANT):**
     The agent must output code changes in markdown code blocks with file paths:
     ```
     Here's my analysis of the crash:
     ...
     The action will:
     1. Parse the code block markers (```filepath.ext` and closing ```)
     2. Extract the code content
     3. Write it to the specified file
     4. Commit the changes automatically
     
     **Supported file types:** `.java`, `.kt`, `.swift`, `.js`, `.ts`, `.py`, `.go`, `.rb`, `.php`, `.cpp`, `.c`, `.h`, `.cs`, `.xml`, `.json`, `.yaml`, `.yml`, `.sh`, `.bash`, and others.
     
     **Example output for a Java fix:**
     ```
     The NullPointerException occurs because `data` parameter is not checked for null.
     ...
     public class MainActivity {
         public void processData(String data) {
             if (data == null) {
                 Log.w("MainActivity", "Data is null");
                 return;
             }
     ...
     This adds a null check before calling toUpperCase().
     ```
...
**Solution:** The agent ran successfully but did not apply code changes. The action parses Claude's output and applies changes from markdown code blocks. Causes and fixes:

1. **Agent output doesn't include code blocks** — Claude must output code in markdown format:
   ```
   ```filepath.ext
   code here
   ```
   ```
   Check the "Log agent output" step in the workflow run to see what Claude generated. Ensure Claude includes fixed code in markdown blocks with the file path.

2. **File paths in code blocks don't match actual files** — The action only applies changes to files that exist. Ensure:
   - File paths in code blocks match actual files in your repo
   - Paths are relative to the repo root
   - Example: `src/MainActivity.java` not `/src/MainActivity.java`

3. **Agent doesn't understand the crash context** — Review the prompt and stack trace for clarity. Ensure the prompt includes enough detail about what needs fixing.

4. **Stack trace doesn't implicate any files** — Ensure `stack-trace` input is provided and includes file names and line numbers.

5. **Debugging** — Add this step in your workflow to see agent output:
   ```yaml
   - name: View agent output
     if: always()
     run: cat /tmp/agent-output.txt
   ```
